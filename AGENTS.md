# Social Security Claim Calculator — context for Codex

This file is loaded automatically at the start of each session. It exists so a fresh Codex can pick up the project without the user re-explaining context every time.
...
- Google Fonts: Inter (UI/display) + JetBrains Mono (numerals), imported via the `<GlobalStyles>` component (intentional — keeps the calculator's font assets self-contained)
...
├─ components/
│  ├─ GlobalStyles.jsx           Inline <style> block (Google Fonts, range thumb, .num/.display, sticky jump nav)
│  ├─ Header.jsx                 Title + lede paragraph
│  ├─ ModeSwitcher.jsx           Three-mode picker + the switch-mode banner
│  ├─ ModeSwitcher.test.jsx
│  ├─ TabNav.jsx                 Calculator / "Why this exists" / "The math" tab strip
│  ├─ TabNav.test.jsx
│  ├─ AboutPage.jsx              Explainer page (worked example, sources)
│  ├─ AboutPage.test.jsx
│  ├─ MathReferencePage.jsx      Formula/source reference page
│  ├─ MathReferencePage.test.jsx
│  ├─ JumpNav.jsx                Sticky side-scroll in-page jump rail for calculator sections
│  ├─ JumpNav.test.jsx           (anchor hrefs + aria-current click behavior)
...
│  ├─ HealthcarePanel.jsx        Healthcare cost picture + FPL / IRMAA ladder
│  ├─ HealthcarePanel.test.jsx
...
│  ├─ ReturnRateSlider.jsx       Shared real-return slider used in inputs + comparison panels
│  ├─ ReturnRateSlider.test.jsx
│  ├─ ShareLinkButton.jsx        Copies current URL (query params + hash anchors coexist)
│  ├─ ShareLinkButton.test.jsx
...
│  ├─ useTheme.js               Light/dark theme preference
│  ├─ useTheme.test.js
│  ├─ useClickToEditNumber.js   Shared click-to-edit numeric field helper
...
`npm test` should always pass before committing. **713 tests across 35 files** as of this writing: math tests in `src/lib/`, hook tests in `src/hooks/`, and React render tests in `src/components/` + `src/App.test.jsx`. Vitest defaults to the node environment for speed; component / hook test files opt into jsdom by adding `// @vitest-environment jsdom` as the first line. Add new tests when adding new math (live in the relevant `*.test.js`) or new components (mirror the file as `*.test.jsx`).
...
- **Node deprecation warning** in `npm test` output: `module.register()` deprecation. Cosmetic — Vitest still works.
- **Node localStorage warning** in some Vitest output: `localStorage is not available because --localstorage-file was not provided`. Cosmetic in this setup; tests stub browser storage where needed and still pass.
...
- **Pre-67 wage comparison + decision signals (PR #35)**: new `WageCompare` panel (every mode) races the same claiming decision at the current pre-67 wage vs two editable alternatives, netting SS + wage take-home − absolute healthcare into one comparable total; a "marginal work" warning flags when extra work keeps only a few cents per dollar (or loses money), switch- and cliff-aware. Plus two `StrategyCompare` signals: a **decisiveness chip** (clear winner vs longevity-dependent close call, colored by the winner) and a **"BY WAGE" robustness lever** (does one strategy win at every wage — computed on the same `finalEarly` basis as the verdict via the now-exported `projectStrategy`, so the two can't contradict). Adversarially reviewed (multi-agent find→verify); 5 confirmed findings fixed, notably a BY-WAGE-vs-verdict basis contradiction at survivor claim ages below 62. Full mechanics under "Wage comparison + decision signals" in the math model section above.
- **Sticky side-scroll jump nav (`JumpNav`)**: a mode-aware in-page navigation rail between `ModeSwitcher` and the calculator grid. Uses real hash anchors for progressive enhancement, `position: sticky` + horizontal overflow for a side-scroll rail, `.jump-target { scroll-margin-top: ... }` so targets land below the rail, and `IntersectionObserver` + `aria-current="location"` to keep the visible section highlighted. Links target the sliders, each summary card (`summary-claim-check`, `summary-fra-check`, optional `summary-post-fra-work`, `summary-result`, optional `summary-healthcare`), chart, survivor strategy comparison when rendered, wage comparison, pot table, healthcare picture, optimum, and sensitivity panel. The active pill recenters inside the horizontal rail on narrow screens. `useUrlSync` preserves `window.location.hash`, so these anchors coexist with shareable query-string state.
- Current verification after the jump-nav work: **713 tests across 35 files**, `npm run lint`, `npm run build`, and browser verification via local Chrome headless / DevTools against `http://127.0.0.1:5173/` (sticky rail stayed pinned, chart jump landed below the rail, mobile rail side-scrolled and kept the active pill readable). The in-app Browser bridge can fail in this Windows sandbox with `windows sandbox failed: spawn setup refresh`; if that happens, local Chrome headless via DevTools is an acceptable fallback for visual checks.
...
- **UI changes**: actually open the page in a browser before declaring it shipped. `curl` returning HTTP 200 only means the JS *parsed*, not that React *rendered*. Production white-page bugs have happened from skipping this. Prefer the in-app Browser plugin when available. On this Windows setup, if the in-app Browser bridge fails, a local Chrome headless / DevTools smoke test against `http://127.0.0.1:5173/` has worked for checking sticky positioning, anchor jumps, screenshots, and console/page errors.
